...
#### The optional printed-number reader

`backend/services/card_ocr.py` is the third optional piece, and the only one that does not look at artwork. It is **off unless explicitly enabled**, by the same pattern as the embedding — the env var both gates the runtime import and drives the build-time install:

```
LOCAL_SCANNER_OCR=1     # installs rapidocr-onnxruntime + opencv-python-headless
```

It reads the `NNN/TTT` printed at the bottom of the card and resolves it against `sets.printed_total` joined on `(tcg_set_id, lang)`. Not against the set code, which the recogniser reads badly and consistently — `DCBL` for a printed `PBL` — while the printed total is plain digits beside the number it has already read. The resolve is also the real filter on a misread: a wrong number almost never lands on a pair the catalogue actually holds.

This exists because two things are outside artwork's reach no matter how good the ranker gets. Roughly a fifth of the catalogue has a name and a number and no picture, so those rows are not in the index at all; and no image score distinguishes "the right card is absent" from "the right card is hard", so a photo of one is answered confidently rather than refused. A (number, printed total) pair identifies a single artwork 64.7% of the time and three or fewer 94% of the time, which is a stronger identity signal than anything else in the system. Because it can name a card that has no artwork, `_apply_printed_number` fetches those rows from the database rather than the index snapshot.

The merge is **additive only**. It promotes and appends candidates and never removes one the artwork search found, because an OCR misread is at least as likely as a wrong image match, and a fail-safe merge is worth more here than a clever one. When it does reorder, the answer stops claiming confidence, since the badge calibration was fitted without this evidence. Answers carry `_printed_numbers`.

A/B'd on a single build with the flag as the only variable, across 87 real photographs: on the 24 whose answer is known the right card went from 17 to 22 at rank 1 and from 22 to 24 in the shortlist, and a further 21 photos now answer with a card that has no artwork at all and so was unreachable by image at any ranker quality. One photo regressed, which is the honest cost of the signal: a correctly read `19/84` also names a Japanese set holding 84 cards, and the wrong one took the lead — the right card stayed in the shortlist, because the merge only ever adds.

The cost is real and is the reason it is opt-in: +186MB of image, and inference that takes the median scan from ~972ms to ~4.3s against ~25ms for the whole fingerprint pipeline. There is no cheap way to skip it on easy scans, because the embedding path never claims confidence in the first place. Measured through the deployed endpoint on 87 real phone photographs, it read a number on 75; on the 25 of those whose printed number is known it read 22 correctly, 2 not at all, and 1 incorrectly. That one matters out of proportion to its count: a `010/064` read as `70/64` is a pair another set really prints, so the plausibility check and the catalogue lookup both passed it and it promoted the wrong card. Most misreads fail to name any card and vanish silently, but not all of them do, which is why the merge is only ever allowed to reorder. `LOCAL_SCANNER_MODEL_THREADS` caps its onnxruntime thread pool too — shared deliberately, since both are onnxruntime contending for the same cores, and left uncapped it fails to pin threads and floods the log on every scan.
