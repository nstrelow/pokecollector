...
The cost is real and is the reason it is opt-in: +311MB of image, and inference that takes the median scan from ~972ms to ~4.3s against ~25ms for the whole fingerprint pipeline. There is no cheap way to skip it on easy scans, because the embedding path never claims confidence in the first place. Measured through the deployed endpoint on 87 real phone photographs, it read a number on 53; on the 25 of those whose printed number is known it read 20 correctly, 5 not at all, and none incorrectly — the failure mode is silence rather than a confident wrong answer, which is what makes it safe to reorder a shortlist on. `LOCAL_SCANNER_MODEL_THREADS` caps its onnxruntime thread pool too — shared deliberately, since both are onnxruntime contending for the same cores, and left uncapped it fails to pin threads and floods the log on every scan.
