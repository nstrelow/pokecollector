...
The toggle can only be switched on once `GET /api/cards/recognize/local/status` reports `ready` (enough of the catalogue fingerprinted); it can always be switched back off. If the index stops being ready afterwards, queued scans retry transiently rather than failing, and resume when the hourly backfill catches up. Because artwork alone carries no text, printings that differ only by language cannot be told apart and are left for the user to pick from the shortlist.
