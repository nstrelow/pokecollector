...
class FingerprintBackfillTests(unittest.TestCase):
    ...
    def test_budget_abort_stops_the_run_immediately_and_only_takes_the_first_reason(self):
        """Unit-level: `abort()` bypasses the time/failure-rate rules entirely."""
        budget = fingerprint_backfill._Budget(deadline=time.monotonic() + 3600)
        self.assertIsNone(budget.check())
        budget.abort("consumer failed")
        self.assertEqual(budget.check(), "consumer failed")
        # A later, unrelated abort must not overwrite the original reason.
        budget.abort("something else")
        self.assertEqual(budget.check(), "consumer failed")

    def test_a_consumer_error_stops_queued_rows_from_still_downloading(self):
        """`flush()` raising must not let `shutdown(wait=True)` drain the queue.

        `ThreadPoolExecutor.map` submits every row up front. Without tripping
        the budget when the consumer fails, exiting the `with` block waits for
        every already-queued row to finish -- each one still a real request --
        before the database error can propagate.
        """
        total = 200
        for i in range(total):
            self._add(f"sv1-{i:03d}_en")
        ...
        def counting_download(client, url, pacer, attempts_arg=4):
            with lock:
                attempts.append(url)
                n = len(attempts)
            return fingerprint_backfill.Download(_png(n))

        with patch.object(fingerprint_backfill, "download_image", counting_download), \
             patch.object(fingerprint_backfill, "COMMIT_BATCH", 1), \
             patch.object(fingerprint_backfill, "_store",
                           side_effect=RuntimeError("db is down")):
            with self.assertRaises(RuntimeError):
                fingerprint_backfill.run_backfill(
                    self.db, rps=1000, workers=2, shuffle=False,
                )

        self.assertLess(
            len(attempts), total // 4,
            "queued rows kept downloading after the consumer had already failed",
        )
