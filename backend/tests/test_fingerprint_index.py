...
class FingerprintIndexTests(unittest.TestCase):
    ...
    def test_a_null_is_custom_or_is_digital_does_not_drop_the_card_silently(self):
        """is_custom/is_digital are nullable columns; NULL must read as FALSE.

        migrate_card_ids() already treats a NULL is_custom as not-custom (`WHERE
        is_custom IS NULL OR is_custom = FALSE`). An equality comparison against
        False in the index's own filter excludes NULL rows instead -- not an
        error, just a card that silently never becomes recognisable.
        """
        self._card("null_custom_en", is_custom=None)
        self._card("null_digital_en", is_digital=None)
        self.db.commit()

        indexed = {row["id"] for row in fingerprint_index.get(self.db).rows}
        self.assertEqual(indexed, {"null_custom_en", "null_digital_en"})
    ...
    def test_reads_are_one_atomic_snapshot(self):
        ...
        WAL, and a busy timeout well above the default five seconds, for the
        same reason: in SQLite's rollback journal a reader blocks a writer, so
        four threads reading as fast as they can starved the writer's commit
        and failed the test with "database is locked" about once in twelve runs
        -- a flake that says nothing about the snapshot invariant under test.
        WAL lets the readers keep their aggression, which is the entire point
        of the test, without them being able to lock the writer out.
        """
        tmpdir = tempfile.mkdtemp()
        try:
            engine = create_engine(
                "sqlite:///" + os.path.join(tmpdir, "snapshot.db"),
            ...
            mismatches = []
            # A reader that raises would otherwise die quietly and leave the
            # test passing on however many threads survived.
            reader_errors = []
    ...
    def test_a_rebuild_does_not_block_everyone_else(self):
        ...
        Uses a file-backed database and one real connection per thread, like
        `test_reads_are_one_atomic_snapshot` above -- not `self.engine`, whose
        StaticPool gives every session in this class the *same* underlying
        SQLite connection. The rebuilder and reader threads below hold open
        transactions concurrently (one blocked inside `slow_load`, one waiting
        on the lock); sharing one connection serialises them onto the same
        unsynchronised transaction state instead of exercising real concurrent
        reads, and can raise `OperationalError`.
        """
        tmpdir = tempfile.mkdtemp()
        try:
            engine = create_engine("sqlite:///" + os.path.join(tmpdir, "rebuild.db"))
            Base.metadata.create_all(engine)
            Session = sessionmaker(bind=engine)
            db = Session()
            db.add_all([
                ...
                Setting(key="tcgdex_digital_sets_enabled", value="false"),
                Card(
                    id="first_en", tcg_card_id="first", name="first_en", number="1",
                    set_id="sv1", lang="en", is_custom=False, is_digital=False,
                    images_small="https://example.invalid/first_en.png",
                    image_phash=_hash(len("first_en")),
                ),
            ])
            db.commit()
            fingerprint_index.get(db)  # prime, so there is something to serve

            db.add(Card(
                id="second_en", tcg_card_id="second", name="second_en", number="1",
                set_id="sv1", lang="en", is_custom=False, is_digital=False,
                images_small="https://example.invalid/second_en.png",
                image_phash=_hash(len("second_en")),
            ))
            db.commit()
            fingerprint_index.invalidate()
            ...
            try:
                def rebuild():
                    session = Session()
                    try:
                        rebuilt.append(len(fingerprint_index.get(session).rows))
                    finally:
                        session.close()
    ...
    def test_the_very_first_load_does_block(self):
        ...
        File-backed database and one real connection per thread, for the same
        reason as `test_a_rebuild_does_not_block_everyone_else` above.
        """
        tmpdir = tempfile.mkdtemp()
        try:
            engine = create_engine("sqlite:///" + os.path.join(tmpdir, "first_load.db"))
            Base.metadata.create_all(engine)
            Session = sessionmaker(bind=engine)
            db = Session()
            db.add_all([
            ...
            try:
                def load_first():
                    session = Session()
                    try:
                        first_result.append(fingerprint_index.get(session))
                    finally:
                        session.close()
                ...
                def load_second():
                    session = Session()
                    try:
                        second_result.append(fingerprint_index.get(session))
                    finally:
                        session.close()
...
class UpsertInvalidationTests(unittest.TestCase):
    ...
    def test_a_field_by_field_write_does_not_null_out_fields_a_brief_payload_omits(self):
        """`parsed` is not always a full-detail response.

        `parse_card_for_db` also runs on the brief /cards listing shape, which
        leaves fields like rarity/types/attacks as `None`. Refreshing an
        existing row from that shape must not erase what a previous full sync
        already populated -- `upsert_set`/`_cache_set_by_code` already skip
        `None` for `Set` rows for the same reason.
        """
        upsert_card(self.db, {
            **self._payload("https://example.invalid/a.png"),
            "rarity": "Rare Holo",
            "types": ["Grass"],
        })
        self.db.commit()
        ...
        apply_catalogue_fields(self.db, card, {
            "id": card.id, "tcg_card_id": "sv1-1", "name": "Card",
            "number": "1", "set_id": "sv1", "lang": "en", "is_custom": False,
            "images_small": "https://example.invalid/a.png",
            "images_large": "https://example.invalid/a.png",
            # A brief listing does not carry these -- they parse to None.
            "rarity": None, "types": None,
        })
        self.db.commit()

        self.assertEqual(card.rarity, "Rare Holo")
        self.assertEqual(card.types, ["Grass"])

    def test_a_field_by_field_write_with_no_images_small_key_keeps_the_fingerprint(self):
        """Omitting the key entirely must not be read as a rotation to None.

        `parsed.get("images_small")` used to return None for a payload that
        never carries the key, which compared unequal to the existing URL and
        cleared a fingerprint that still matched the (unchanged) artwork.
        """
        upsert_card(self.db, self._payload("https://example.invalid/a.png"))
        self.db.commit()
        self.db.query(Card).update({
        ...
        apply_catalogue_fields(self.db, card, {
            "id": card.id, "tcg_card_id": "sv1-1", "name": "Card",
            "number": "1", "set_id": "sv1", "lang": "en", "is_custom": False,
            # No "images_small" key at all.
        })
        self.db.commit()

        self.assertEqual(card.images_small, "https://example.invalid/a.png")
        self.assertEqual(card.image_phash, _hash(3))
        self.assertEqual(card.image_phash_source, "https://example.invalid/a.png")
