...
def apply_catalogue_fields(db: Session, existing: Card, parsed: dict) -> Card:
    ...
    `parsed` is not always a full-detail response -- `parse_card_for_db` also
    accepts the brief listing shape (id/localId/name/image), which leaves most
    other fields `None`. `upsert_set`/`_cache_set_by_code` already skip `None`
    for the same reason on `Set` rows; without the same guard here, refreshing
    a card from a brief listing would null out rarity, types, attacks and
    everything else a previous full sync had populated.
    """
    if "images_small" in parsed and parsed["images_small"] is not None:
        clear_stale_fingerprint(existing, parsed["images_small"])
