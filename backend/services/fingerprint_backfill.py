...
class _Budget:
    ...
    def abort(self, reason: str) -> None:
        """Stop the run immediately, regardless of the time/failure-rate rules.

        For a failure that has nothing to do with either -- a consumer-side
        error such as `flush()` raising on a database error -- so that
        `work()`'s `budget.check()` starts returning early instead of letting
        every already-queued task in `pool.map` finish downloading before
        `ThreadPoolExecutor.__exit__`'s `shutdown(wait=True)` lets the
    ...
    with httpx.Client(
        ...
        with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
            ...
            try:
                ...
                # Otherwise shutdown(wait=True) below downloads the rest of the
                # batch -- each still a real TCGdex request -- before this can
                # propagate, up to a full time_budget after storage already
                # failed.
